...
const GET_MODEL_EVALUATION_REGION_TAG = 'automl_translate_get_model_evaluation';
const LIST_OPERATION_STATUS_REGION_TAG = 'automl_translate_list_operation_status';
const GET_OPERATION_STATUS_REGION_TAG = 'automl_translate_get_operation_status';

const CREATE_MODEL_REGION_TAG = 'automl_translate_create_model';
const DELETE_MODEL_REGION_TAG = 'automl_translate_delete_model';
...
describe('Automl Translate Model Tests', () => {
  ...
  it('should list and get model, list and get model evaluations', async () => {
    ...
    // list models
    const list_model_output = execSync(`node automl/${LIST_MODEL_REGION_TAG}.js ${projectId}`);
    assert.match(list_model_output, /Model id:/);
    const modelId = list_model_output.split('Model id: ')[1].split('\n')[0];

    // get model
    const get_model_output = execSync(`node automl/${GET_MODEL_REGION_TAG}.js ${projectId} ${modelId}`);
    assert.match(get_model_output, /Model id/);

    // list model evaluations
    const list_model_eval_output = execSync(`node automl/${LIST_MODEL_EVALUATION_REGION_TAG}.js ${projectId} ${modelId}`);
    ...
    const get_model_eval_output = execSync(`node automl/${GET_MODEL_EVALUATION_REGION_TAG}.js ${projectId} ${modelId} ${modelEvaluationId}`);
  ...
  it('should list / get operation status', async () => {
    ...
    const list_output = execSync(`node automl/${LIST_OPERATION_STATUS_REGION_TAG}.js ${projectId}`)
    assert.match(list_output, /Operation details/);
    const operation_id = list_output.split('Name: ')[1].split('\n')[0];

    const get_output = execSync(`node automl/${GET_OPERATION_STATUS_REGION_TAG}.js ${operation_id}`)
    assert.match(get_output, /Operation details/);
  });

  // it('should create a model', async () => {
  //   const projectId = await client.getProjectId();
  //   const create_output = execSync(`node automl/${CREATE_MODEL_REGION_TAG}.js ${projectId} ${DATASET_ID} translation_test_create_model`)
